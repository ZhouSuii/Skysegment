...
RESULTS_DIR = "results"
CONFIGS_DIR = "configs"
BEST_PARAMS_FILE = os.path.join(CONFIGS_DIR, "best_gnn_dqn_params.json")
...
def objective(trial: optuna.Trial):
    """GNN-DQN超参数调优的Optuna目标函数"""
    run_start_time = time.time()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"试验 {trial.number}: 开始于设备 {device}")

    # 1. 建议超参数
    # GNN-DQN特有的搜索空间，包括图卷积相关参数
    config = {
        'learning_rate': trial.suggest_float('learning_rate', 1e-5, 1e-2, log=True),
        'hidden_dim': trial.suggest_categorical('hidden_dim', [64, 128, 256, 512]),  # GNN隐藏层维度
        'num_layers': trial.suggest_int('num_layers', 1, 3),  # GNN层数
        'dropout_rate': trial.suggest_float('dropout_rate', 0.0, 0.5),  # Dropout率
        'gamma': trial.suggest_float('gamma', 0.9, 0.99),  # 折扣因子
        'epsilon_decay': trial.suggest_float('epsilon_decay', 0.99, 0.999),  # 探索率衰减
        'batch_size': trial.suggest_categorical('batch_size', [32, 64, 128, 256]),  # 批量大小 - GNN通常需要小一些的批量
        'target_update_freq': trial.suggest_int('target_update_freq', 5, 20),  # 目标网络更新频率
        # 固定参数以提高调优效率
        'epsilon': 1.0,  # 初始探索率固定为1.0
        'epsilon_min': 0.01,  # 最小探索率固定
        'memory_size': 5000,  # 经验回放容量
        'jit_compile': False  # 对于GNN模型，JIT编译可能不总是有效，因此默认禁用
    }
    ...
    try:
        ...
        # 初始化GNN-DQN代理 - 这里不需要像普通DQN那样明确指定状态和动作空间大小
        agent = GNNDQNAgent(graph, NUM_PARTITIONS, config=config)
        ...
        if device.type == 'cuda':
            ...
            for _ in range(5):
                ...
                state = next_state
            if agent.current_memory_size >= agent.batch_size:
                agent.replay()  # 进行一次小批量的学习
            env.reset()  # 重置环境，准备开始正式训练
...
if __name__ == "__main__":
    # 创建或加载研究
    study_name = "gnn-dqn-partitioning-optimization"
    ...
    if best_trial:  # 仅当存在最佳试验时才绘图
        try:
            ...
            history_path = os.path.join(OPTUNA_PLOTS_DIR, "optuna_gnn_dqn_history.png")
            importance_path = os.path.join(OPTUNA_PLOTS_DIR, "optuna_gnn_dqn_importance.png")
            ...
            parallel_path = os.path.join(OPTUNA_PLOTS_DIR, "optuna_gnn_dqn_parallel.png")
